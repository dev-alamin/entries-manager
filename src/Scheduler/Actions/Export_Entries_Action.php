<?php

namespace Amin\FormsEntriesManager\Scheduler\Actions;

defined( 'ABSPATH' ) || exit;

use Amin\FormsEntriesManager\Api\Callback\Export_Entries;

class Export_Entries_Action {



	protected $export_entries;

	public function __construct( Export_Entries $export_entries ) {
		$this->export_entries = $export_entries;

		add_action( Export_Entries::BATCH_PROCESSING_HOOK, array( $this->export_entries, 'process_export_batch' ), 10, 1 );

		add_action( Export_Entries::FINALIZE_HOOK, array( $this->export_entries, 'finalize_export_file' ), 10, 1 );

		add_action( 'entr_mgr_daily_cleanup', array( $this, 'entr_mgr_clean_old_exports' ) );

		if ( ! wp_next_scheduled( 'entr_mgr_daily_cleanup' ) ) {
			wp_schedule_event( time(), 'daily', 'entr_mgr_daily_cleanup' );
		}
	}

	/**
	 * Removes old CSV export files from the 'entr_mgr_exports' directory.
	 *
	 * This method scans the 'entr_mgr_exports' folder within the WordPress uploads directory
	 * and deletes any CSV files that are older than 2 days based on their last modification time.
	 *
	 * @return void
	 */
	public function entr_mgr_clean_old_exports() {
		$dir = wp_upload_dir()['basedir'] . '/entrydashboard-exports';
		foreach ( glob( $dir . '/*.csv' ) as $file ) {
			if ( filemtime( $file ) < strtotime( '-2 days' ) ) {
				wp_delete_file( $file );
			}
		}
	}
}
