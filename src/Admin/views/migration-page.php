...
<div x-data="migrationHandler()" x-init="init()"
	class="wrap entr-mgr-admin-page min-h-screen !max-w-7xl !m-auto bg-white px-8 py-10 text-[15px] font-inter text-gray-800 space-y-10">
	...
	<div class="mb-8 bg-slate-700 !text-white px-4 py-2 rounded-lg">
		<h1 class="!text-3xl !font-extrabold !tracking-tight !flex !items-center !text-white !gap-3">
			🚀 <span><?php esc_html_e( 'Migrate from WPFormsDB', 'entries-manager' ); ?></span>
		</h1>
		<p class="!text-gray-200 !mt-1">
			<?php esc_html_e( 'Easily migrate your old WPFormsDB entries to take full advantage of our advanced features.', 'entries-manager' ); ?>
		</p>
	</div>
	...
	<div class="bg-indigo-50 border border-indigo-200 p-6 rounded-lg space-y-4">
		<h2 class="text-lg font-semibold text-indigo-700">
			<strong><?php esc_html_e( 'Total Entries:', 'entries-manager' ); ?></strong>
		...
		<div class="bg-white rounded-md border border-gray-200 shadow-sm p-4">
			<h2 class="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
				...
				<?php esc_html_e( 'Migration Overview', 'entries-manager' ); ?>
			</h2>
		...
		<template x-if="totalEntries > 10000">
			<p class="text-sm text-red-600">
				<?php esc_html_e( '⚠️ You have a large dataset. We recommend using a batch size of 100 or less to avoid timeouts.', 'entries-manager' ); ?>
			</p>
		</template>

		<div class="flex items-center gap-4">
			<span class="text-gray-700 text-sm"><?php esc_html_e( 'Batch Size:', 'entries-manager' ); ?></span>
			<template x-if="totalEntries > 10000">
				<label class="block">
					<div class="relative mt-1 w-32">
						<input type="number" min="10" max="100" x-model.number="batchSize"
							...
							placeholder="<?php echo esc_attr__( '50', 'entries-manager' ); ?>"
			...
			<button @click="startMigration"
				...
				<?php esc_html_e( 'Start Migration', 'entries-manager' ); ?>
			</button>

			<!-- See Progress Button -->
			<button @click="seeProgress"
				title="<?php esc_attr_e( 'View Migration Progress', 'entries-manager' ); ?>"
				x-show="!migrating && !complete && migrationInProgress"
				...
				<?php esc_html_e( 'Migration is running. See Progress', 'entries-manager' ); ?>
			</button>
			...
			<button @click="stopMigration"
				...
				<?php esc_html_e( 'Stop Migration', 'entries-manager' ); ?>
			</button>
			...
			<span x-show="complete" class="text-green-600 font-medium flex items-center gap-1 text-sm">
				...
				<?php esc_html_e( 'Migration Completed!', 'entries-manager' ); ?>
			</span>
		</div>
	</div>
	...
	<div x-show="migrating" class="mt-4 space-y-2 bg-indigo-50 p-6 pb-8 rounded-lg">
		<h2 class="text-lg font-semibold mb-2"><?php esc_html_e( 'Migration Progress', 'entries-manager' ); ?></h2>
		<!-- Label -->
		<p class="text-sm text-gray-700 font-medium">
			<?php esc_html_e( 'Migrating Entries:', 'entries-manager' ); ?> <span class="font-bold" x-text="migrated"></span> /
			<span x-text="total"></span>
		</p>

		<!-- Estimated Time -->
		<p class="text-sm text-gray-500 italic" x-show="estimatedTime">
			<?php esc_html_e( 'Estimated time left:', 'entries-manager' ); ?> <span x-text="estimatedTime"></span>
		</p>

		<!-- Progress Label -->
		<div class="mb-1 text-sm text-gray-700 font-medium flex justify-between">
			<span><?php esc_html_e( 'Progress:', 'entries-manager' ); ?></span>
	...
	<template x-if="log.length">
		<div class="mt-4 space-y-2 bg-indigo-50 p-6 rounded-lg pb-8">
			<h2 class="text-lg font-semibold mb-2"><?php esc_html_e( 'Migration Log', 'entries-manager' ); ?></h2>
