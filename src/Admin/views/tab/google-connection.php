...
<section class="">
	<div class="bg-white shadow-md border border-indigo-100 p-8 text-center">
		<h3 class="!text-2xl font-semibold !mb-4">
			...
			esc_html_e( 'Google Account Connection', 'entries-manager' );
			?>
		</h3>
			...
			<div class="max-w-md mx-auto bg-green-50 border border-green-200 rounded-xl px-6 py-4 text-green-800 shadow-md transition-transform transform hover:scale-105">
				<div class="flex items-center space-x-3">
					...
					<span class="text-base font-semibold text-green-900">
						<?php esc_html_e( 'Connected Google Sheets', 'entries-manager' ); ?>
					</span>
				</div>
					...
					if ( ! empty( $forms ) ) {
						foreach ( $forms as $form_id ) {
							...
							if ( $form_title && $spreadsheet_id ) {
								...
								echo '<span>' . esc_html_e( 'View Sheet', 'entries-manager' ) . '</span>';
								echo '</a>';
								echo '</div>';
							}
						}
		...
		<a
		title="<?php esc_attr_e( 'Revoke Google Connection. It won\'t remove your sheets just no sync anymore.', 'entries-manager' ); ?>"
		href="
			<?php
			echo esc_url(
				admin_url(
					add_query_arg(
						...
						'admin.php?page=entrydashboard-entries-manager-settings'
					)
				)
			);
			?>
