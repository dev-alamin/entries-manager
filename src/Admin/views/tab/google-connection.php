...
<section class="">
	<div class="bg-white shadow-md border border-indigo-100 p-8 text-center">
		<h3 class="!text-2xl font-semibold !mb-4">
			...
			esc_html_e( 'Google Account Connection', 'entrydashboard' );
			?>
		</h3>
			...
			<div class="flex justify-center !mb-6">
				<div class="flex flex-col items-center gap-4 p-6 rounded-xl bg-green-50 border border-green-200 text-green-800 shadow-md max-w-md mx-auto">
					...
					<div class="flex items-center gap-3">
						...
						<p class="font-semibold text-lg m-0"><?php esc_html_e( 'Connected to Google Sheets', 'entrydashboard' ); ?></p>
					</div>
					...
					<div class="flex flex-col items-center gap-2 mt-2">
						...
						class="text-green-700 hover:text-green-900 underline font-semibold text-sm">
							<?php esc_html_e( 'Switch Account', 'entrydashboard' ); ?>
						</a>
						<span class="text-sm text-green-700 mt-2"><?php esc_html_e( 'Live data sync is active. Streaming enabled ✅', 'entrydashboard' ); ?></span>
					</div>
				</div>
			</div>
			
			<p class="!mb-6 text-gray-600 max-w-2xl mx-auto text-center !m-auto">
				<?php esc_html_e( 'Your WPForms submissions are now syncing automatically with your Google Sheets in real-time. This connection allows you to streamline your data collection and analysis.', 'entrydashboard' ); ?>
			</p>
			...
			<div class="max-w-md mx-auto bg-green-50 border border-green-200 rounded-xl px-6 py-4 text-green-800 shadow-md transition-transform transform hover:scale-105">
				<div class="flex items-center space-x-3">
					...
					<span class="text-base font-semibold text-green-900">
						<?php esc_html_e( 'Connected Google Sheets', 'entrydashboard' ); ?>
					</span>
				</div>
					...
					if ( ! empty( $forms ) ) {
						foreach ( $forms as $form_id ) {
							...
							if ( $form_title && $spreadsheet_id ) {
								...
								echo '<span>' . esc_html_e( 'View Sheet', 'entrydashboard' ) . '</span>';
								echo '</a>';
								echo '</div>';
							}
						}
					} else {
						echo '<p class="text-sm text-green-600">' . esc_html__( 'No forms are currently connected to Google Sheets.', 'entrydashboard' ) . '</p>';
					}
					?>
				</div>
			</div>
		...
		<a
		title="<?php esc_attr_e( 'Revoke Google Connection. It won\'t remove your sheets just no sync anymore.', 'entrydashboard' ); ?>"
		href="
			<?php
			echo esc_url(
				admin_url(
		...
		class="inline-flex mt-4 items-center justify-center gap-2 px-7 py-3 rounded-lg bg-red-600 hover:bg-red-700 !text-white font-medium shadow transition max-w-xs mx-auto">
			...
			<?php esc_html_e( 'Revoke Connection', 'entrydashboard' ); ?>
		</a>
		
		<!-- No access token then show connection way -->
		<?php else : ?>
			<p class="!mb-6 text-gray-600">
				<?php esc_html_e( 'To start syncing WPForms entries with Google Sheets, please connect your Google account. This will enable live synchronization and easy data management.', 'entrydashboard' ); ?>
			</p>
			...
			"
				...
				🔐 <?php esc_html_e( 'Connect with Google', 'entrydashboard' ); ?>
			</a>
		<?php endif; ?>
	</div>
</section>