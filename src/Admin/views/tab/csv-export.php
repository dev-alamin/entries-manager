...
" class="bg-white border border-indigo-100 shadow p-6 space-y-6 rounded-lg">
	...
	<!-- Form Selector -->
	<div>
		<label for="entr_mgr_export_form" class="block text-sm font-semibold text-gray-700 mb-1">
			<?php esc_html_e( 'Select Form', 'entries-manager' ); ?>
		</label>
		<select id="entr_mgr_export_form"
			name="entr_mgr_export_form"
			...
			<option value=""><?php esc_html_e( '-- Select a Form --', 'entries-manager' ); ?></option>
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
		<div>
			<label for="entr_mgr_export_date_from" class="block text-sm font-semibold text-gray-700 mb-1">
				<?php esc_html_e( 'Date From', 'entries-manager' ); ?>
			</label>
			<input type="date" id="entr_mgr_export_date_from" name="entr_mgr_export_date_from"
				class="!w-full !px-4 !py-2 !border !border-gray-300 !rounded-md !shadow-sm !text-sm !focus:ring !focus:ring-indigo-200 !focus:border-indigo-500">
		</div>
		<div>
			<label for="entr_mgr_export_date_to" class="block text-sm font-semibold text-gray-700 mb-1">
				<?php esc_html_e( 'Date To', 'entries-manager' ); ?>
			</label>
			<input type="date" id="entr_mgr_export_date_to" name="entr_mgr_export_date_to"
				class="!w-full !px-4 !py-2 !border !border-gray-300 !rounded-md !shadow-sm !text-sm !focus:ring !focus:ring-indigo-200 !focus:border-indigo-500">
		</div>
	</div>
	...
	<div x-show="fields.length > 0">
		<label class="block text-sm font-semibold text-gray-700 mb-2">
			<?php esc_html_e( 'Exclude Columns (Optional)', 'entries-manager' ); ?>
		</label>
	...
	<div class="pt-4 space-y-2">
		<button
			...
			<template x-if="isOntheGoExport">
				<span>
					<?php esc_html_e( 'Start Exporting', 'entries-manager' ); ?>
				</span>
			</template>
			<template x-if="!isExporting && !isOntheGoExport">
				<span><?php esc_html_e( 'Start Exporting', 'entries-manager' ); ?></span>
			</template>
		...
		<button
			...
			<span x-text="isExportComplete ? '<?php esc_html_e( 'Download Export File', 'entries-manager' ); ?>' : '<?php esc_html_e( 'See Export Progress', 'entries-manager' ); ?>'"></span>
		</button>
		...
		<button
			...
			<?php esc_html_e( 'Delete Export File', 'entries-manager' ); ?>
		</button>
	</div>
	...
	<div
		...
		<div @click.away="closeProgressModal()" class="bg-white rounded-lg shadow-xl p-6 w-96 max-w-full">
			<div class="flex justify-between items-center mb-4">
				<h3 class="text-lg font-semibold text-gray-800"><?php esc_html_e( 'Export Progress', 'entries-manager' ); ?></h3>
			...
			<div class="mb-4">
				<p class="text-sm text-gray-600 mb-2"><?php esc_html_e( 'The export is currently in progress. You can close this window and it will continue in the background.', 'entries-manager' ); ?></p>
				...
				<div class="mt-2 text-sm font-medium text-gray-700">
					...
					<span x-show="exportProgress > 0" class="float-right text-gray-500">
						...
						<?php esc_html_e( 'Entries Processed', 'entries-manager' ); ?>
					</span>
				</div>
			</div>
			...
			<div class="flex justify-end space-x-2">
				<button @click="closeProgressModal()" class="px-4 py-2 text-sm font-semibold text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300">
					<?php esc_html_e( 'Close', 'entries-manager' ); ?>
				</button>
			</div>
		</div>
	</div>
