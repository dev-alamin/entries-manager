...
<div
	...
	<div class="bg-white border border-indigo-100 shadow p-6">
		<h3 class="text-xl font-bold text-indigo-700 mb-3"><?php esc_html_e( '📄 Pagination Settings', 'entries-manager' ); ?></h3>
		<p class="text-sm text-gray-600 mb-4">
			<?php esc_html_e( 'Set how many entries are shown per page in the admin UI.', 'entries-manager' ); ?>
		</p>

		<div>
			<label for="entr_mgr_entries_per_page" class="block text-sm font-medium text-gray-700 mb-1">
				<?php esc_html_e( 'Entries per Page', 'entries-manager' ); ?>
			</label>
			<input type="number" name="entr_mgr_entries_per_page" id="entr_mgr_entries_per_page"
	...
	<div class="bg-white border border-indigo-100 shadow p-6">
		<h3 class="text-xl font-bold text-indigo-700 mb-3">
			<?php esc_html_e( '📊 Table Columns', 'entries-manager' ); ?>
		</h3>
		<p class="text-sm text-gray-600 mb-4">
			<?php esc_html_e( 'Select which form fields you want to display as columns in the entries table.', 'entries-manager' ); ?>
		</p>
			...
			<div class="mb-6 border border-gray-200 rounded-md p-4">
				...
				<div class="space-y-2 max-h-48 overflow-y-auto border border-gray-200 p-4 rounded-md">
					<template x-for="field in allFields[form.form_id]" :key="field.key">
						<div class="flex items-center">
							...
							<input
								...
								:name="'entr_mgr_custom_columns[' + form.form_id + '][]'"
					...
					<p x-show="!allFields[form.form_id]" class="text-gray-500 text-xs mt-1">
						<?php esc_html_e( 'Loading fields...', 'entries-manager' ); ?>
					</p>
				</div>
			</div>
		</template>
