// ENTR_MGR_I18N: The global object containing all translatable strings.
// This object is generated by wp_localize_script in PHP.
// Example content: entrMgrStrings = { "csvExportedSuccess": "CSV exported successfully!", ... }
...
function formTable(form) {
  return {
    ...
    entries: [],
    currentPage: 1,
    pageSize: entrMgrSettings.perPage,
    totalPages: 1,
    sortAsc: true,
    sortAscStatus: true,
    dateFrom: "",
    ...
    selectedEntry: {},
    bgClasses: ["entr-mgr-row-bg-1", "entr-mgr-row-bg-2", "entr-mgr-row-bg-3", "entr-mgr-row-bg-4"],
      ...
      try {
        if (action === "export_csv") {
          const res = await fetch(
            `${entrMgrSettings.restUrl}entrydashboard/v1/export/bulk`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "X-WP-Nonce": entrMgrSettings.nonce,
              },
              body: JSON.stringify({
          ...
          a.href = url;
          a.download = `entr-mgr-entries-${Date.now()}.csv`;
          ...
          this.$dispatch("toast", {
            type: "success",
            // ENTR_MGR_I18N: Use translatable string
            message: entrMgrStrings.csvExportedSuccess,
          });
        } else {
          ...
          const res = await fetch(
            `${entrMgrSettings.restUrl}entrydashboard/v1/entries/bulk`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
        ...
        this.$dispatch("toast", {
          type: "error",
          message: entrMgrStrings.bulkActionFailed + " " + error.message,
        });
      }
    },
    ...
    async fetchEntries() {
      ...
      try {
        const res = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/entries?${query}`,
          {
            headers: {
              "X-WP-Nonce": entrMgrSettings.nonce,
            },
        ...
        const formChoices = entrMgrSettings.initialColumns[this.formId] || [];;
        ...
        this.$dispatch("toast", {
            type: "error",
            message: entrMgrStrings.fetchEntriesError,
         });        
      } finally {
        this.loading = false;
      }
    ...
    copyEntryToClipboard() {
        ...
        navigator.clipboard
            ...
            .catch((err) => {
                this.$dispatch("toast", {
                    type: "error",
                    message: entrMgrStrings.copyFailed,
                });
            });
    },
    ...
    async updateEntry(index, changes = {}) {
      ...
      try {
        const res = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/entries/${
            payload.id
    ...
    async deleteEntry() {
      ...
      try {
        ...
        if (data.deleted) {
          ...
          this.$dispatch("toast", {
            type: "error",
            message: entrMgrStrings.deleteFailedUnknown.replace(
              "Unknown error",
        ...
        this.$dispatch("toast", {
          type: "error",
          message: entrMgrStrings.deleteRequestFailed,
        });
      }
    },
    ...
    async updateSelectedEntry(changes = {}) {
      ...
      if (index === -1) {
        // ENTR_MGR_I18N: Use translatable string
        console.error(entrMgrStrings.entryNotFound);
            this.$dispatch("toast", {
                type: "error",
                message: data.message,
          });
      ...
      try {
        ...
        this.$dispatch("toast", {
          type: "success",
          // ENTR_MGR_I18N: Use translatable string
          message: entrMgrStrings.changesSavedSuccess,
        });
        ...
        // ENTR_MGR_I18N: Use translatable string
            this.$dispatch("toast", {
                type: "error",
                message: entrMgrStrings.saveFailed,
          });
      }
    },
    validateAndSaveNote() {
      ...
      if (note.length > 1000) {
        // ENTR_MGR_I18N: Use translatable string
            this.$dispatch("toast", {
                type: "error",
                message: entrMgrStrings.noteTooLong,
          });
        return;
      }
   ...
   async toggleGoogleSheetSync(index) {
    ...
    timeAgo(dateString) {
      ...
      if (seconds < 60) return entrMgrStrings.timeAgoJustNow;
      if (seconds < 3600)
        return entrMgrStrings.timeAgoMinutes.replace(
          "%d",
          Math.floor(seconds / 60)
        );
      if (seconds < 86400)
        return entrMgrStrings.timeAgoHours.replace(
          "%d",
      ...
      if (
        ...
        return entrMgrStrings.timeAgoYesterday;
      }
...
function entriesApp() {
  return {
    ...
    async fetchForms() {
      this.loading = true;
      try {
        const res = await fetch(`${entrMgrSettings.restUrl}entrydashboard/v1/forms`, {
          headers: {
            "X-WP-Nonce": entrMgrSettings.nonce,
          },
        });
...
function settingsForm() {
  return {
    ...
    async saveSettings() {
      ...
      const form = document.querySelector("#entr-mgr-settings-form");
      ...
      formData.append("action", "entr_mgr_save_settings");
      formData.append("_wpnonce", entrMgrSettings.nonce);
      ...
      try {
        ...
        if (data.success) {
          window.dispatchEvent(
            new CustomEvent("toast", {
              detail: {
                // ENTR_MGR_I18N: Use translatable string
                message: entrMgrStrings.settingsSavedSuccess,
                type: "success",
              },
            })
          );
          ...
          window.dispatchEvent(
            new CustomEvent("toast", {
              detail: {
                // ENTR_MGR_I18N: Use translatable string
                message:
                  entrMgrStrings.saveFailed + " " + (data.data?.message || ""),
                type: "error",
              },
            })
          );
        ...
        // ENTR_MGR_I18N: Use translatable string
        this.message = entrMgrStrings.unexpectedError;
      }
...
function exportSettings() {
  return {
    ...
    init() {
      ...
      const savedJobId = localStorage.getItem("entr_mgr_export_job_id");
      if (savedJobId) {
    ...
    async fetchForms() {
      try {
        const res = await fetch(`${entrMgrSettings.restUrl}entrydashboard/v1/forms`, {
          headers: { "X-WP-Nonce": entrMgrSettings.nonce },
        });
        const data = await res.json();
        this.forms = data;
      } catch (e) {
    ...
    async fetchFormFields() {
      ...
      try {
        const res = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/forms/${this.selectedFormId}/fields`,
          {
            headers: { "X-WP-Nonce": entrMgrSettings.nonce },
          }
        );
    ...
    async exportAllBatchesAsync() {
      if (!this.selectedFormId) {
        this.errorMessage = entrMgrStrings.selectFormExport;
        return;
      }
      ...
      this.dateFrom = document.getElementById("entr_mgr_export_date_from").value;
      this.dateTo = document.getElementById("entr_mgr_export_date_to").value;
      ...
      try {
        const res = await fetch(`${entrMgrSettings.restUrl}entrydashboard/v1/export/start`, {
          method: "POST",
          headers: {
            "X-WP-Nonce": entrMgrSettings.nonce,
            "Content-Type": "application/json",
        ...
        if (contentType.includes("text/csv")) {
          ...
          if (text.startsWith("id,") && text.includes("\n")) {
            ...
            const a = document.createElement("a");
            a.href = url;
            a.download = `entr-mgr-entries-${this.formTitle}-${Date.now()}.csv`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            ...
            throw new Error(entrMgrStrings.invalidCSVContent);
          }
        }
        ...
        if (res.ok && result.success) {
          ...
          localStorage.setItem("entr_mgr_export_job_id", this.exportJobId);
          this.startPolling();
        } else {
    ...
    startPolling() {
      ...
      this.exportInterval = setInterval(async () => {
        try {
          const res = await fetch(
            `${
              entrMgrSettings.restUrl
            }entrydashboard/v1/export/progress?job_id=${encodeURIComponent(
              this.exportJobId
            )}`,
            {
              headers: { "X-WP-Nonce": entrMgrSettings.nonce },
    ...
    handleDownload(fileUrl) {
      if (!this.exportJobId) {
        this.errorMessage = entrMgrStrings.cannotDownloadCSV;
        return;
      }
      // This implementation is fine. It uses a dedicated endpoint which is good practice.
      const downloadUrl = `${
        entrMgrSettings.restUrl
      }entrydashboard/v1/export/download?job_id=${encodeURIComponent(this.exportJobId)}`;
    ...
    resetExportState() {
      ...
      localStorage.removeItem("entr_mgr_export_job_id");
    },
    ...
    async deleteExportFile() {
      ...
      try {
        const res = await fetch(
          `${
            entrMgrSettings.restUrl
          }entrydashboard/v1/export/delete?job_id=${encodeURIComponent(this.exportJobId)}`,
          {
            method: "POST",
            headers: {
              "X-WP-Nonce": entrMgrSettings.nonce,
...
function migrationHandler() {
  return {
    ...
    init() {
      ...
      const savedBatch = localStorage.getItem("entr_mgr_batch_size");
      ...
      if (!this.entryFetchStarted) {
        ...
        fetch(`${entrMgrSettings.restUrl}entrydashboard/v1/legacy/source/count`, {
          headers: { "X-WP-Nonce": entrMgrSettings.nonce },
        })
          .then((res) => {
      ...
      const inProgress = localStorage.getItem("entr_mgr_migration_in_progress");
      if (inProgress === "true") {
    ...
    async startMigration() {
      ...
      localStorage.setItem("entr_mgr_migration_in_progress", "true");
      localStorage.setItem("entr_mgr_batch_size", this.batchSize);

      try {
        const triggerRes = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/migration/trigger`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!triggerData.success) {
          ...
          localStorage.setItem("entr_mgr_migration_in_progress", "false");
          this.log.push(
    ...
    async checkProgress() {
      try {
        const res = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/migration/progress`,
          {
            headers: { "X-WP-Nonce": entrMgrSettings.nonce },
          }
        );
...
function customColumnsForm() {
  return {
    ...
    init() {
      ...
      if (
        typeof entrMgrSettings !== "undefined" &&
        entrMgrSettings.initialColumns
      ) {
        this.selectedColumns = entrMgrSettings.initialColumns;
      }
    },
    ...
    async fetchForms() {
      ...
      try {
        const response = await fetch(`${entrMgrSettings.restUrl}entrydashboard/v1/forms`, {
          headers: { "X-WP-Nonce": entrMgrSettings.nonce },
        });
        const data = await response.json();
        this.forms = data;
    ...
    async fetchFields(form_id) {
      ...
      try {
        const response = await fetch(
          `${entrMgrSettings.restUrl}entrydashboard/v1/forms/${form_id}/fields`,
          {
            headers: { "X-WP-Nonce": entrMgrSettings.nonce },
          }
        );
...
document.addEventListener('alpine:init', () => {
    Alpine.data('reviewNotice', () => ({
        ...
        dismissNotice(type) {
            ...
            formData.append('action', `entr_mgr_dismiss_review_request_${type}`);
            formData.append('_wpnonce', entrMgrReviewData.nonce); // Use the localized nonce

            fetch(entrMgrReviewData.ajax_url, {
                method: 'POST',
                body: formData,
